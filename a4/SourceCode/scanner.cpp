...
void Scanner::init_digits(unordered_map<char, int> &umap, int target){
    for(int i = 0; i < 10; i++){
        umap[char('0'+i)] = target;
    }
}
void Scanner::init_letters(unordered_map<char, int> &umap, int target){
    for(int i = 0; i < 26; i++){
        umap[char('a'+i)] = target;
        umap[char('A'+i)] = target;
    }
}
void Scanner::init_l_and_d(unordered_map<char, int> &umap, int target){
...
// Initialization
Scanner::Scanner(string file_name) : print_index(0) {
        ...
        while(c_file.get(next_char)){
            ...
            // The token is read completely
                ...
                if(final_state_map.find(current_state) != final_state_map.end()){
                    ...
                    if(current_state != 29){
                        tokens.push_back(token(final_state_map[current_state], current_str, -1));
                    }
